...
type Client struct {
	...
	log             logging.Logger
	getMessage      func(ctx context.Context, uint65 uint64) (messengerpb.Message, uint64)
...
func NewClient(client messengerpb.MessengerClient, log logging.Logger, getMessage func(ctx context.Context, uint65 uint64) (messengerpb.Message, uint64)) *Client {
...
func (c *Client) handleRequest(req *messengerpb.EventRequest, stream messengerpb.Messenger_NotifyClient) {
	if req.GetStart() {
		c.dispatchStart(stream, req.GetPChainHeight())
		return
	}
...
func (c *Client) dispatchStart(stream messengerpb.Messenger_NotifyClient, pChainHeight uint64) {
	...
	go func() {
		defer wg.Done()
		c.sendMessageBack(ctx, stream, pChainHeight)
	}()
}

func (c *Client) sendMessageBack(ctx context.Context, stream messengerpb.Messenger_NotifyClient, pChainHeight uint64) {
	// We ignore the returned P-chain height as the RPC VM cannot have the P-chain height, only the embedded VM can.
	msg, _ := c.getMessage(ctx, pChainHeight)
