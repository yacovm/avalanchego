...
| `--skip-to-snowman` | `AVAGO_SKIP_TO_SNOWMAN` | boolean | `false` | Skip state sync and bootstrapping on every chain and immediately start consensus from the locally stored last accepted block. The node will not catch up with the network before participating in consensus, so this should only be used when the local database is already at or near the network tip. |
...
| `--min-stake-duration` | `AVAGO_MIN_STAKE_DURATION` | duration | network dependent | Minimum staking duration. This can only be changed on a local network. This applies to both delegation and validation periods. Defaults to `336h` (2 Weeks) on Mainnet. Defaults to `24h` (1 Day) on Test Net. |
| `--helicon-min-stake-duration` | `AVAGO_HELICON_MIN_STAKE_DURATION` | duration | network dependent | Minimum staking duration post Helicon. This can only be changed on a local network. This applies to only to validation periods. Defaults to `48h` (2 Days) on Mainnet. Defaults to `1h` (1 Hour) on Test Net. |
