...
func addNodeFlags(fs *pflag.FlagSet) {
	...
	fs.Bool(SkipToSnowmanKey, false, "Skip state sync and bootstrapping on every chain and immediately start consensus from the locally stored last accepted block. The node will not catch up with the network before participating in consensus")
